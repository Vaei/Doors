...
public:
	/** Pack the door state and door side into a single uint8 for replication */
	static EReplicatedDoorState PackDoorState(EDoorState DoorState, EDoorSide DoorSide);
	
	/** Unpack the door state and door side from a single uint8 from replication */
	static void UnpackDoorState(EReplicatedDoorState DoorStatePacked, EDoorState& OutDoorState, EDoorSide& OutDoorSide);
	 ...
	 * @param AvatarDoorSide The side of the door that we are standing on -- this is not the door's side itself
	 * @param NewDoorState The new resulting state of the door
	 * @param NewDoorSide The new resulting side of the door
	...
	static bool ProgressDoorState(const ADoor* Door, EDoorState DoorState, EDoorSide AvatarDoorSide,
